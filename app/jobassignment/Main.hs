...
solve = \ case
    cs -> case unzip cs of
        (as,bs) -> case (minimum as, minimum bs) of
            (am, bm) -> case (elemIndices am as, elemIndices bm bs) of
                ([i],[j])
                    | i == j -> minimum [ am + bm
                                        , am `max` minimum (delete bm bs)
                                        , minimum (delete am as) `max` bm
                                        ]
                _            -> max am bm
