...
type Solver = (Integer,Int,Int) -> O

solve :: Solver
solve = \ case
    (y,m,d) -> iter (fromGregorian y m d) where
        iter day = case toGregorian day of
            (yy,mm,dd) | yy `mod` toInteger (mm * dd) == 0 -> printf "%04d/%02d/%02d" yy mm dd
                       | otherwise                         -> iter (succ day)


wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    [s]:_ -> case f (h $ toTriple (map read (g s))) of
        r     -> [[r]]
    _   -> error "wrap: invalid input format"
    where
        g s = words (map conv s)
        conv '/' = ' '
        conv c   = c
        h (i,j,k) = (toInteger i,j,k)
