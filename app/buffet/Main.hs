...
solve :: Solver
solve = \ case
    (n,as,bs,cs) -> case (listArray (1,n) bs, listArray (0,pred n) (0:cs)) of
        (ba, ca) -> fst $ foldl' phi (0,0) as where
            phi (s,i) a = (s + ba ! a + c, a) where
                c = if succ i /= a then 0 else ca ! i

wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    [n]:as:bs:cs:_ -> case f (n,as,bs,cs) of
        r -> [[r]]
